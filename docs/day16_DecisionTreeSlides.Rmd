...
- what is accuracy if we randomly assign "no defaults" vs "default"?
...
- How do we find the *best* partition (tree) of the data to make a decision tree?
- Typical algorithm is **recursive binary partitioning**
    + **top down**: starts at the top node of the tree with all of the data cases, successively splits the data (binary splits at each node)
    + **greedy**: picks the *best* split at the current node, doesn't look ahead to determine which current split would produce a better tree in a future step
    + **results**: get a "locally" optimum tree, rather than a globally optimum tree
...
- Earlier I restricted the treee depth to 2
...
- Ex: Predict meat consumption by income (by country)
